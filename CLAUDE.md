...
The production file uses UTF-8. **Never use PowerShell `Out-File`** to write or copy
`index.html` — it re-encodes to UTF-16 LE and corrupts multi-byte characters (£, ✓, →, ×).

Always use:
```powershell
[System.IO.File]::Copy()          # for byte-perfect copy
...
- `.gitignore` — Codex set this deliberately; `.claude/` is intentionally excluded
