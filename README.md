...
├── README.md
└── docs/
    ├── visible-spec-v1.7.md       # Full product and architecture spec
    ├── visible-brief-v1.7.md      # One-page brief and emotional design contract
    └── visible-positioning-v1.md  # Market positioning paper
```

> **Note:** Docs reference v1.7 but the app currently runs as BETA v1.8. Alignment is a known pending task.
...
> **Security note:** API calls currently go directly from the browser using the `anthropic-dangerous-direct-browser-access` header. Keys are stored in localStorage. A lightweight backend proxy is planned to remove key exposure from the browser. See [docs/visible-spec-v1.7.md](docs/visible-spec-v1.7.md) for roadmap.
...
The app is two files: `index.html` and `sw.js`. Deploy both to any static web host.
...
2. FTP `index.html` and `sw.js` to Fasthosts webspace
...
- **No icon / web manifest** — PWA install experience is incomplete; `/icon.png` referenced but not present
...
See [docs/visible-spec-v1.7.md](docs/visible-spec-v1.7.md) for the full product spec.

Immediate priorities (stabilisation):
- [ ] HTML escaping for dynamic content (XSS protection)
- [ ] AI backend proxy (remove key from browser)
- [ ] Extract and test calculation logic
- [ ] Fix service worker / add icons / web manifest
- [ ] Align docs to v1.8
